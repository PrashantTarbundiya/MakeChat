...
export const PromptInputBox = React.forwardRef((props, ref) => {
  ...
  const models = [
    { id: 'llama-maverick', name: 'Llama Maverick' },
    { id: 'gpt-oss', name: 'GPT OSS' },
    { id: 'llama-scout', name: 'Llama Scout' },
    { id: 'nemotron-super', name: 'Nemotron 120B' },
  ...
  const isDocFile = (file) => ['application/pdf', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document', 'text/plain'].includes(file.type);
  ...
  const processFile = (file) => {
    ...
    if (!isImageFile(file) && !isDocFile(file)) {
      alert("Only image and document files are allowed");
      return;
    }
    ...
    if (isImageFile(file)) {
      ...
      setFilePreviews(prev => ({ ...prev, [file.name]: 'document' }));
    }
  };
  ...
  const handleDrop = React.useCallback((e) => {
    ...
    const validFiles = files.filter((file) => isImageFile(file) || isDocFile(file));
          ...
          <div className="flex flex-wrap gap-2 p-0 pb-1 transition-all duration-300">
            {files.map((file, index) => (
              <div key={index} className="relative group">
                {file.type.startsWith("image/") && filePreviews[file.name] && filePreviews[file.name] !== 'document' ? (
                  <div
                ...
                <input
                  ...
                  onChange={(e) => {
                    if (e.target.files && e.target.files.length > 0) {
                      Array.from(e.target.files).slice(0, 5 - files.length).forEach(file => processFile(file));
                    }
